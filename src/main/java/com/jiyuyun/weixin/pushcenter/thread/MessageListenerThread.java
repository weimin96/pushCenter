...
public class MessageListenerThread extends Thread {
    ...
    public void run() {
        while (true) {
            ...
            log.info("监听消息队列");
            String msgJson = (String) redisTemplate.opsForList().rightPop(Constant.WX_MESSAGE_KEY, 10000, TimeUnit.MILLISECONDS);
            if (StringUtils.isNotBlank(msgJson)) {
                ...
                log.info("消费消息<{}>,剩余次数<{}>", message.getContent(), message.getRemainCount());
            }
        }
    }
